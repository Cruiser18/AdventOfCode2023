...
let isFiveOfAKind (input: string) =
    ...
    input.[1..] |> Seq.forall (fun c -> c = firstChar || c = 'J')

let isFourOfAKind (input: string) =
    let numJChars = countJChars input
    let charCounts =
        input
        |> Seq.groupBy id
    ...
    charCounts
    |> Map.exists (fun _ count -> count + numJChars = 4 || numJChars = 4)
...
let isFullHouse (input: string) =
    ...
    charCounts
    |> Map.exists (fun _ count -> count + numJChars = 3 || count = 3)
    && charCounts
        |> Map.exists (fun _ count -> count + numJChars = 2 || count = 2)
...
let isThreeOfAKind (input: string) =
    ...
    charCounts
    |> Map.exists (fun _ count -> count + numJChars = 3 || numJChars = 3)
let countJChars (input: string) =
    input
...
let isTwoPairs (input: string) =
    ...
    charCounts
    |> Map.filter (fun _ count -> count + numJChars = 2 || numJChars = 2 || count = 2)
    |> Map.count = 2
